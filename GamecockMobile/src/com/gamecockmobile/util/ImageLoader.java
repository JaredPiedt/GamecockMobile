...
/**
 * A class that wraps up remote image loading request using the Volley library combined with a
 * memory cache. An single instance of this class should be created once when your Activity or
 * Fragment is created, then us {@link #get(String, android.widget.ImageView)} or one of the
 * variations to queue the image to be fetched and loaded from the network. Loading images in a
 * {@link android.widget.ListView} or {@link android.widget.GridView} is also supported but you must
 * store the {@link com.android.volley.Request} in you ViewHolder type class and pass it into
...
public class ImageLoader extends com.android.volley.toolbox.ImageLoader {
  ...
  /**
   * Creates an ImageLoader with Bitmap memory cache and a default placeholder image while the image
  ...
  /**
   * Stops processing request on the {@link RequestQueue}.
   */
  ...
  public ImageContainer get(String requestUrl, ImageView imageView, int placeHolderIndex) {
    return get(requestUrl, imageView, mPlaceHolderDrawables.get(placeHolderIndex), mMaxImageWidth,
        mMaxImageHeight);
  }
  ...
  public ImageContainer get(String requestUrl, ImageView imageView, Drawable placeHolder,
    ...
    ImageContainer imageContainer = imageView.getTag() != null
        && imageView.getTag() instanceof ImageContainer ? (ImageContainer) imageView.getTag()
        : null;

    // Find image url from prior request
    String recycledImageUrl = imageContainer != null ? imageContainer.getRequestUrl() : null;

    // If the new requestUrl is null or the new requestUrl is different to the previous recycled
    // requestUrl
    if (requestUrl == null || !requestUrl.equals(recycledImageUrl)) {
      ...
      if (requestUrl != null) {
        ...
        imageContainer = get(requestUrl,
            getImageListener(mResources, imageView, placeHolder, mFadeInImage), maxWidth, maxHeight);
      } else {
  ...
  private static RequestQueue newRequestQueue(Context context) {
    ...
    Network network = new BasicNetwork(UIUtils.hasHoneycomb() ? new HurlStack()
  ...
  /**
   * Sets a {@link android.graphics.Bitmap} to an {@link android.widget.ImageView} using a fade-in
   * animation. If there is a {@link android.graphics.drawable.Drawable} already set on the
   * ImageView then use that as the image to fade from. Otherwise fade in from a transparent
   * Drawable.
   */
  ...
  private static void setImageBitmap(final ImageView imageView, final Bitmap bitmap,
    ...
    if (fadeIn && UIUtils.hasHoneycombMR1()) {
      ...
      final TransitionDrawable td = new TransitionDrawable(new Drawable[] { initialDrawable,
          bitmapDrawable });
  ...
  public static File getDiskCacheDir(Context context, String uniqueName) {
    // Check if the media is mounted or storage is built-in, if so, try and use external cache dir
    // otherwise use internal cache dir
    final String cachePath = Environment.MEDIA_MOUNTED
        .equals(Environment.getExternalStorageState()) || !Environment.isExternalStorageRemovable() ? getExternalCacheDir(
        context).getPath()
   ...
   */
   ...
   public interface ImageLoaderProvider {
     public ImageLoader getImageInstance();
   }
}
